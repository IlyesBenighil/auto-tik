...
def create_zoom_video(image_path, output_path, duration=5.0, zoom_factor=1.5, fps=30, 
    ...
    logger.info(f"Création d'une vidéo de zoom à partir de {image_path}")
    logger.info(f"Durée: {duration}s, Zoom: x{zoom_factor}, FPS: {fps}, Mouvement: {movement} ({movement_type})")
    if blur > 0:
        logger.info(f"Flou constant appliqué avec une intensité de {blur}")
    ...
    logger.info(f"Génération et écriture de {num_frames} frames...")
    ...
    for i in range(num_frames):
        ...
        if i % max(1, num_frames // 10) == 0 or i % 100 == 0:
            logger.info(f"Progression: {i}/{num_frames} frames ({i/num_frames*100:.1f}%)")
    ...
    try:
        ...
        logger.info("Conversion du format vidéo pour une meilleure compatibilité...")
        ...
        logger.warning(f"Impossible de convertir le format vidéo: {str(e)}")
        logger.warning(f"La vidéo reste au format original: {output_path}")
